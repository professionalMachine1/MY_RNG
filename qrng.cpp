...
{
  float_type res;
  res = modf(h1 * alpha2 + h2 * alpha + cur_num * h3, &int_val);
  
  h1 = modf(cur_num * h1 * alpha + h2, &int_val);
  h2 = modf(cur_num * h2 * alpha2 + h1, &int_val);
  h3 = modf(cur_num * h3 * alpha + h1, &int_val);
